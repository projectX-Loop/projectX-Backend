...
class GlobalExceptionHandlerTests {
	...
	void marksUnavailableDependenciesAsRetryable() {
		...
		assertThat(response.getBody()).isEqualTo(new ApiError("PLAN-502-1", "Calculator is unavailable", true, List.of()));
	}
	...
	void returnsGenericErrorForUnexpectedRuntimeException() {
		...
		assertThat(response.getBody()).isEqualTo(new ApiError("COMMON-500", "Internal server error", true, List.of()));
	}

	@Test
	void returnsFrontendErrorJsonWithoutSuccessWrapper() throws Exception {
		MockMvc mockMvc = MockMvcBuilders
				.standaloneSetup(new PlanController(mock(PlanCreateService.class)))
